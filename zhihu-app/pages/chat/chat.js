...
Page({
    ...
    wx.getStorage({
      ...
      success(res) {
        ...
        wx.request({
          ...
          success(res2) {
            if (res2.data.code == 0) { //说明请求成功，把返回的数据，设置给data
              console.log("获取到的会话列表", res2.data)
              console.log(res.data.id)
              that.setData({
                session: res2.data.data
              })
            } else { //失败  提示   失败原因
              console.log("会话列表获取失败")
            }
  ...
  toMessage: function (event) {
    ...
    wx.navigateTo({
      url: 'message/message?sendid=' + event.currentTarget.dataset.sendid + '&receiveid=' + event.currentTarget.dataset.receiveid + '&username=' + event.currentTarget.dataset.username,
    })
  }
})